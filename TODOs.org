...
*** logic
**** get the path of all files and folders recursivelly
**** filter only files
**** create 3 lists: videos, subs, files
**** for each file:
***** if the extension is a video extension
video extensions =
.WEBM
...
**** do the same as above to subs
**** do the same as above to files
**** for each of the lists move/rename the files from src > dst
** TODO get_videos_info_csv.py
*** logic
**** check if the folder has the right structure
contains: videos/
**** get every video into a list
**** for each video:
***** create an empty dict
***** get the full path of the file
***** get length_sec, width, height, aspect_ratio using openCV
***** convert length_sec to length_hour
***** get the accumulated_duration_in_seconds
***** convert accumulated_duration_in_seconds in accumulated_duration_in_hours
***** get the size of the video
***** get the accumulated_size
***** add all the variables to the dict
***** put the dict in place of the element
**** create the csv file
**** for each video
...
** TODO remember how to cleanly get dict values into csv file
